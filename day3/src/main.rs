...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    for line in lines {
        if let Ok(l) = line {
            ...
            let dup = sack
                .find_duplicate()
                .expect("should always be a duplicate in a valid sack");
    ...
    println!(
        "Total of priorities of duplicated sack items was {}",
        total_dup_prios
    );
    println!(
        "Total of priorities of badges in each elf group was {}",
        total_badge_prios
    );
    Ok(())
}
